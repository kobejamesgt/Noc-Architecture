...
struct packet
{
	...
	packet(int sx = -1, int sy = -1, int dx = -1, int dy = -1,int rou=1,char *cont="non",
		token_type t = token_type())
		: src_x(sx), src_y(sy), dest_x(dx), dest_y(dy),round(rou),content(cont), token(t)
	{
	}
	...
	bool operator==(const packet &x) const
	{
		return (x.src_x == src_x) && (x.src_y == src_y)
			&& (x.dest_x == dest_x) && (x.dest_y == dest_y)&& (x.round == round)&& (x.content == content)
			&& (x.token.real == token.real)&&
			   (x.token.imag == token.imag); 
	}
}; // struct packet
